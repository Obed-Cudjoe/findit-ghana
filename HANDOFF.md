...
1. **Supabase** — production path. Free tier is enough. Setup: create project → run `supabase/migrations/001_init.sql`, then `002_featured_listings.sql`, then `003_vendor_plans.sql`, then `004_vendor_passwords.sql`, then `005_unlimited_plan.sql`, then `006_listing_images.sql` (product-photo column + public `vendor-images` storage bucket), then `007_vendor_listing_updates.sql` (adds `vendor_listings.updated_at` so vendor price edits refresh the "Prices checked" freshness) → add the three env vars. Done.
...
Flow: vendor registers the shop on `/for-vendors` (lands in `/admin/vendors` as pending, nothing in the listings queue) → signs in at `/vendor` → adds products on `/vendor/listings` with 3–6 photos each, up to the plan cap → each product goes to `/admin/listings` for approval. Vendors can then **edit price / stock / delivery fee / delivery days / description** of their own listings from the **Edit** button on `/vendor/listings` — approved edits go live immediately (no re-review) and the "Prices checked" timestamp on the product page uses the edit time (`vendor_listings.updated_at`, added by `007_vendor_listing_updates.sql`).

Paid flow: vendor picks a paid plan at signup → pays MoMo to **053 126 2424** → WhatsApps the reference → you confirm on **`/admin/vendors`** → **MoMo → Starter 30d**, **MoMo → Pro 30d** or **MoMo → Unlimited 30d** (sets plan + 30-day expiry). Directory: `/vendors` and `/vendors/[slug]`. Vendor login: **`/vendor`** (cookie `findit_vendor`, 12h). Same-named products from different shops merge onto one product page (vendor comparison table). If you use Supabase, run `003_vendor_plans.sql`, then `004_vendor_passwords.sql`, then `005_unlimited_plan.sql`, then `006_listing_images.sql`, then `007_vendor_listing_updates.sql` after 001 and 002. Optional `VENDOR_JWT_SECRET` (falls back to `ADMIN_PASSWORD`). **Product photos (3–6 per listing)** upload to the public `vendor-images` storage bucket in Supabase mode, to `public/uploads/` on a dev machine, and are refused (with a clear message) on the public demo store — so photos need Supabase to work on a live Vercel deploy. Every upload is **auto-compressed server-side** (longest side ≤ 1600 px, JPEG/WebP quality ~80 — a ~3 MB phone photo lands well under ~500 KB stored), and the browser downscales large photos before sending to save vendor upload data.

**Deleting a shop (suspected fraud / bad actor).** On **`/admin/vendors`** each shop row has a red **Delete shop** action that requires a two-step confirm naming the shop ("This removes the shop, all its listings and its photos, and cannot be undone"). It deletes, in order: all the shop's listings (vendor_id match plus the legacy phone-key match), its uploaded photos (Supabase Storage objects under `listings/<slug>/…`, or the local `public/uploads/listings/<slug>/` folders in dev), then the `vendor_profiles` row. The deleted shop leaves the `/vendors` directory and search immediately, and its dashboard login stops working (valid cookie, no profile → redirect to login). **Click events and buyer reports are kept** as the audit trail. Official price sources (Jumia, CompuGhana, Franko, Telefonika) live in JSON catalogues, not `vendor_profiles` — the Delete action only ever applies to actual shop rows.
