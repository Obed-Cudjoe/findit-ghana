  ...
  return (
    <div>
      ...
      <h2 className="mt-8 text-lg font-extrabold text-navy-900">
        Price-drop alerts {triggeredAlerts.length > 0 && <span className="ml-2 rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-bold text-amber-800">{triggeredAlerts.length} ready to send</span>}
      </h2>
            ...
            return (
              <li key={a.id} className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1 px-4 py-3 text-sm">
                <span className="min-w-0">
                  <span className="font-semibold text-navy-900">{a.productName}</span>
                  <span className="ml-2 text-xs text-slate-soft">target GH₵{a.targetPriceGhs.toLocaleString("en-GH")} · +{a.phone} · triggered {a.triggeredAt ? new Date(a.triggeredAt).toLocaleDateString("en-GB", { day: "numeric", month: "short" }) : "—"}</span>
                </span>
                <a
                  href={waLink}
                  target="_blank"
