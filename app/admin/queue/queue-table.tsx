      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[720px] text-sm">
          ...
          <tbody className="divide-y divide-navy-100">
            {rows.map((r) => (
              <tr key={r.id}>
                <td className="px-4 py-3 font-mono text-xs font-bold text-navy-900 dark:text-navy-100">{r.refCode}</td>
                <td className="px-4 py-3 font-semibold text-navy-800 dark:text-navy-200">{r.kindLabel}</td>
                <td className="max-w-xs px-4 py-3 text-xs text-slate-soft dark:text-navy-300">
                  <p className="line-clamp-2">{r.detail}</p>
                  {r.listingUrl && <a href={r.listingUrl} target="_blank" rel="noopener noreferrer" className="text-navy-500 dark:text-navy-400 underline">listing link ↗</a>}
                </td>
                <td className="px-4 py-3 text-xs text-slate-soft dark:text-navy-300">{new Date(r.createdAt).toLocaleString("en-GB", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}</td>
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-wrap justify-end gap-1.5">
                    ...
                    <button disabled={pending} onClick={() => setStatus(r.id, "fixed")} className="rounded-md border border-emerald-200 px-2.5 py-1 text-xs font-semibold text-emerald-700 dark:text-emerald-300 hover:bg-emerald-50 transition-colors">Fixed</button>
