  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-10">
      ...
      <nav aria-label="Breadcrumb" className="text-sm text-slate-soft">
        ...
        <span className="text-navy-900">{guide.title}</span>
      </nav>

      <header className="mt-4">
        <h1 className="text-2xl font-extrabold leading-tight text-navy-900 md:text-3xl">{guide.title}</h1>
        <p className="mt-3 flex items-center gap-2 text-xs text-slate-soft">
          <Clock className="h-3.5 w-3.5 text-gold-600" /> Updated {new Date(guide.updatedAt).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })} · {guide.readMinutes} min read
        </p>
      </header>
      ...
      <div className="prose-simple mt-6 space-y-4 text-[15px] leading-relaxed text-ink">
        {guide.body.split("\n").map((line, i) => {
          if (line.startsWith("## ")) {
            return <h2 key={i} className="pt-4 text-xl font-extrabold text-navy-900">{line.replace("## ", "")}</h2>;
          }
          if (line.trim() === "") return null;
          return <p key={i} className="text-slate-soft">{line}</p>;
        })}
      </div>

      {/* live comparison table inside the article (COMP-08 reuse) */}
      {firstProduct && (
        <section className="mt-8 rounded-2xl border border-navy-100 bg-navy-50/40 p-4 md:p-6">
          <h2 className="text-lg font-extrabold text-navy-900">Live prices: {firstProduct.name}</h2>
          <p className="mb-4 mt-1 text-xs text-slate-soft">
      ...
      {related.length > 1 && (
        <section className="mt-8">
          <h2 className="text-lg font-extrabold text-navy-900">Products mentioned in this guide</h2>
      ...
      <section className="mt-10 border-t border-navy-100 pt-6">
        <h2 className="text-lg font-extrabold text-navy-900">Keep reading</h2>
        <ul className="mt-3 space-y-2">
          {others.map((g) => (
            <li key={g.slug}>
              <Link href={`/guides/${g.slug}`} className="group inline-flex items-center gap-1.5 text-sm font-semibold text-navy-700 hover:text-gold-700">
