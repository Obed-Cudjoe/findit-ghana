  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      <header className="max-w-2xl">
        <h1 className="text-3xl font-extrabold text-navy-900">Price guides for Ghanaian shoppers</h1>
        <p className="mt-2 text-sm text-slate-soft">Buying advice, scam warnings and price comparisons — updated as prices change.</p>
      </header>

      {featured && (
        <Link href={`/guides/${featured.slug}`} className="hover-lift group mt-8 block overflow-hidden rounded-2xl border border-navy-100 bg-white md:grid md:grid-cols-[280px_1fr]">
          ...
          <div className="p-6">
            <p className="text-xs font-bold uppercase tracking-widest text-gold-700">Featured guide</p>
            <h2 className="mt-1 text-xl font-extrabold text-navy-900 group-hover:text-navy-600 transition-colors">{featured.title}</h2>
            <p className="mt-2 text-sm text-slate-soft">{featured.excerpt}</p>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        {rest.map((g) => (
          <Link key={g.slug} href={`/guides/${g.slug}`} className="hover-lift group overflow-hidden rounded-xl border border-navy-100 bg-white">
            ...
            <div className="p-4">
              <h2 className="font-bold text-navy-900 group-hover:text-navy-600 transition-colors">{g.title}</h2>
