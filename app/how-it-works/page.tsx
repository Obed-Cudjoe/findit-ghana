  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10">
      <header className="text-center">
        <h1 className="text-3xl font-extrabold text-navy-900">How it works</h1>
        <p className="mx-auto mt-3 max-w-xl text-slate-soft">Three steps between you and the honest price. No account. No prepayment to us. No surprises.</p>
      </header>
      ...
      <div className="mt-10 space-y-6">
        {[
          ...
          <div key={s.title} className="flex gap-4 rounded-xl border border-navy-100 bg-white p-6">
            <s.icon className="h-8 w-8 shrink-0 text-gold-600" strokeWidth={1.6} aria-hidden="true" />
            <div>
              <h2 className="font-bold text-navy-900">{s.title}</h2>
              <p className="mt-1 text-sm text-slate-soft">{s.text}</p>
            </div>
          </div>
        ))}
      </div>

      <h2 className="mt-12 text-xl font-extrabold text-navy-900">On every listing you&apos;ll see</h2>
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        {[
          ...
          <div key={t.label} className="rounded-xl bg-navy-50 p-5">
            <t.icon className="h-6 w-6 text-navy-700" strokeWidth={1.6} aria-hidden="true" />
            <p className="mt-2 font-bold text-navy-900">{t.label}</p>
            <p className="mt-1 text-sm text-slate-soft">{t.text}</p>
          </div>
        ))}
      </div>

      <h2 className="mt-12 text-xl font-extrabold text-navy-900">What we never do</h2>
      <div className="mt-4 rounded-xl border border-navy-100 p-6">
