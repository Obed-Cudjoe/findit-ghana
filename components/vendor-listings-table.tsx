...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="mb-1 block text-xs font-semibold text-navy-900 dark:text-navy-100">{label}</span>
      {children}
    </label>
  );
}
  ...
  return (
    <form onSubmit={save} noValidate className="space-y-3 rounded-xl border border-gold-500/50 bg-gold-500/5 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm font-bold text-navy-900 dark:text-navy-100">Edit “{listing.productName}”</p>
      ...
      {error && (
        <p className="rounded-lg border border-red-200 bg-red-50 dark:bg-red-900/30 px-3 py-2 text-xs font-medium text-red-700 dark:text-red-300" role="alert">
          {error}
        </p>
      )}
      <div className="flex flex-wrap gap-2">
        ...
        <button
          ...
          onClick={onCancel}
          disabled={saving}
          className="inline-flex items-center gap-1.5 rounded-lg border border-navy-200 dark:border-navy-700 px-4 py-2 text-sm font-semibold text-navy-700 dark:text-navy-300 hover:bg-navy-50 disabled:opacity-60 transition-colors"
        >
...
export function VendorListingsTable({ listings }: { listings: VendorListing[] }) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-navy-100 dark:border-navy-800 bg-white dark:bg-navy-900">
      <p className="border-b border-navy-100 dark:border-navy-800 bg-navy-50/60 dark:bg-navy-900/50 px-4 py-2 text-xs text-slate-400 lg:hidden">Swipe the table sideways →</p>
                ...
                <Fragment key={l.id}>
                  <tr className={editing ? "bg-gold-500/5" : ""}>
                    <td className="px-4 py-3">
                      ...
                      <p className="text-xs text-slate-soft dark:text-navy-300">{l.category}</p>
                    </td>
                    <td className="px-4 py-3">
                      {photos.length > 0 ? (
                        <span className="flex items-center gap-1.5">
                          ...
                          <img
                            ...
                            className="h-10 w-10 rounded-md border border-navy-100 dark:border-navy-800 object-cover"
                            loading="lazy"
                          />
                          <span className="text-xs font-semibold text-slate-soft dark:text-navy-300">×{photos.length}</span>
                        </span>
                      ) : (
                        <span className="text-xs text-slate-400">—</span>
                      )}
                    </td>
                    <td className="px-4 py-3 font-semibold text-navy-900 dark:text-navy-100">GH₵{l.priceGhs.toLocaleString()}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-right">
                      {editing ? (
                        <span className="text-xs font-semibold text-slate-soft dark:text-navy-300">editing…</span>
                      ) : (
                        <span className="inline-flex flex-wrap justify-end gap-1.5">
                          {l.status === "approved" && (
                            <button
                              ...
                              className="inline-flex items-center gap-1 rounded-md border border-emerald-200 px-2.5 py-1 text-xs font-semibold text-emerald-700 dark:text-emerald-300 hover:bg-emerald-50 disabled:opacity-60 transition-colors"
                            >
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-1 rounded-md border border-navy-200 dark:border-navy-700 px-2.5 py-1 text-xs font-semibold text-navy-700 dark:text-navy-300 hover:border-gold-500 hover:text-gold-700 transition-colors"
                          >
                  ...
                  {savedId === l.id && (
                    <tr>
                      <td colSpan={6} className="bg-emerald-50 dark:bg-emerald-900/30 px-4 py-2">
                        <p className="flex items-center gap-1.5 text-xs font-semibold text-emerald-800 dark:text-emerald-200" role="status">
