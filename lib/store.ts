...
import type { ReportRow, ContactRow, ClickRow, VendorListing } from "@/lib/types";
...
export async function readClicks(): Promise<ClickRow[]> {
  const supabase = getSupabase();
  if (supabase) {
    const { data, error } = await supabase.from("click_events").select("*").order("created_at", { ascending: false }).limit(200);
